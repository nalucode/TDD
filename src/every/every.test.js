...
it("every([8,10,40,48], (item) => item % 2 === 0) should return true", () => {
  expect(every([8, 10, 40, 48], (item) => item % 2 === 0)).to.be.ok;
});

it("every([1,2,3], (item,index) => index < item) should return true", () => {
  expect(every([1, 2, 3], (item, index) => index < item)).to.be.ok;
});
